...
- 정식 서명 릴리즈는 승인과 인증서 연결 후 진행한다. 승인 전 공개 배포는 아래 버전별 명시적 배포 결정에 한정한다.
...
## v1.3.19 미서명 배포 결정 (2026-09-20)

- 유지관리자가 v1.3.19의 수정 내용, 테스트 330개 통과, 미서명 설치 후보와 공개 전 상태를 확인한 뒤 "공개배포하자"라고 지시했다. 이 최신 명시적 지시에 따라 이번 v1.3.19 공개 배포를 진행한다.
- 검증된 로컬 설치 파일, 동일 설치 파일을 담은 ZIP, `SHA256SUMS.txt`를 수동으로 GitHub Release에 게시한다. 홈페이지와 릴리즈 노트에 미서명 및 Foundation 승인 대기 상태를 표시한다.
- 기존 파일·태그와 정식 서명 CI는 유지한다. 승인 후에는 앱과 설치 파일을 모두 서명·검증하여 새 버전과 새 태그로 배포한다.
- 이번 결정은 v1.3.19에 한정하며 이후 버전의 미서명 배포를 자동으로 허용하지 않는다.

JEV(`typesafe/jev-1.13-20260917`)에 기존 조항, 사용자 지시와 수정 후보를 실제 전달했다.
응답은 적용 범위를 v1.3.19로 분류했고, 새 문구 내부 충돌과 기존 파일·서명 CI 약화 질문의
응답 값은 각각 0.18, 0.05였다. 최신 사용자 지시와 버전별 예외 범위가 분명하므로 위 문구를
채택했다. 이 수치는 정답률이나 실행 권한으로 사용하지 않았다.

영문 Code signing policy의 같은 예외 범위도 JEV로 추가 검토했다. 응답은 두 버전만
허용하는 것으로 분류했고 한국어 조항과의 범위 충돌 응답 값은 0.20이었다.
두 문서의 버전 목록을 일치시키고 향후 자동 허용과 서명 CI 약화를 금지하는 문구를 유지했다.
